...
if uploaded_file:
    with st.spinner("正在处理空白行并解析..."):
        try:
            ...
            # 再次清理 NaN 较多的行（防止残留空行）
            df = df.dropna(thresh=5).reset_index(drop=True)   # 至少要有5个非空值才保留
            ...
            if main_col is None or sub_col is None:
                ...
                for idx, row in df.iterrows():
                    ...
                    if not main_part or not subpart or main_part.lower() == 'nan' or subpart.lower() == 'nan':
                        debug.append(f"Row {idx}: Main/Sub 仍为空 → 跳过")
                        continue
                    ...
                    for col_idx in range(len(row)-1, 15, -1):
                        ...
                        if cell and cell.lower() != 'nan' and cell != '' and len(cell) > 2:
                            if not any(x in cell for x in ['/202', '4501', 'New Awarded', 'Normal', 'No Job']):
