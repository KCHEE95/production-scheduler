        ...
        try:
            ...
            for idx in st.session_state.progress.index:
                ...
                if row['status'] in ['pending', 'in_progress']:
                    ...
                    waiting_days = (now - arrival).days
                    if waiting_days >= 2:
                        st.session_state.progress.at[idx, 'delay_days'] = waiting_days
        except:
            pass
        time.sleep(4 * 3600)  # 每4小时检查一次
...
def calculate_eta(item_id):
    ...
    last_step = progress.iloc[-1]
    if last_step['status'] == 'completed':
        return last_step['actual_completion'][:16]
    
    dept = last_step['dept']
    delay = last_step.get('delay_days', 0) or 0
    ...
    for _, p in pending.iterrows():
        item = st.session_state.items[st.session_state.items['item_id'] == p['item_id']]
        if not item.empty:
            wf = json.loads(item.iloc[0]['workflow'])
            for step in wf:
                if step['dept'] == dept:
                    total_hours += step['est_hours']
                    break
    
    capacity = 10.5  # 默认每日产能
    dept_row = st.session_state.depts[st.session_state.depts['dept_code'] == dept]
    if not dept_row.empty:
        capacity = dept_row.iloc[0]['daily_capacity_hours']
...
if page == "📤 导入 Epicor Excel":
    ...
    if uploaded_file:
        with st.spinner("正在解析 Excel，请稍等..."):
            ...
            for _, row in df.iterrows():
                ...
                if not main_part or not subpart or main_part == 'nan' or subpart == 'nan':
                    continue
...
# ====================== 其他页面（简化版） ======================
    ...
    with col2:
        st.metric("已完成 Subpart", len(st.session_state.progress[st.session_state.progress['status'] == 'completed']))
    with col3:
        st.metric("部门数量", len(st.session_state.depts))
    
    st.subheader("瓶颈预警（卡住超过2天）")
    delayed = st.session_state.progress[st.session_state.progress.get('delay_days', 0) > 2]
    if not delayed.empty:
        st.error(f"⚠️ 当前有 {len(delayed)} 个 Subpart 处于卡住状态")
        st.dataframe(delayed[['item_id', 'dept', 'delay_days']])
    else:
    ...
    st.title("📊 生产经理仪表板")
    st.subheader("各部门负载情况")
    summary = []
    for _, d in st.session_state.depts.iterrows():
        pending = len(st.session_state.progress[
            (st.session_state.progress['dept'] == d['dept_code']) & 
            (st.session_state.progress['status'] != 'completed')
        ])
        summary.append([d['dept_code'], pending])
    
    summary_df = pd.DataFrame(summary, columns=["部门", "待处理数量"])
    st.bar_chart(summary_df.set_index("部门"))
    ...
    st.title("系统设置")
    st.subheader("部门产能设置")
    edited_depts = st.data_editor(st.session_state.depts, use_container_width=True)
    if st.button("保存设置"):
        st.session_state.depts = edited_depts
        save_data()
        st.success("已保存")

st.sidebar.caption("混合版 | 每4小时自动检测卡住 | 每个步骤固定8小时")
